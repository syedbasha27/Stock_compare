...
`{"status":"ok",...,"groq_key_set":false}`.

## 2b. Set the Groq key (for the agent chat panel)

Secrets never go in `render.yaml` or the repo. Instead:
1. In your Render service → **Environment** tab
2. Add: `GROQ_API_KEY` = `<your key from console.groq.com>`
3. Render redeploys automatically. Re-check `/api/health` —
   `groq_key_set` should now say `true`.

Everything else on the dashboard works without this key; only the
"Ask the index analyst" chat panel needs it.
